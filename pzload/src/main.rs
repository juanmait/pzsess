...
fn main() {
    ...
    if std::path::Path::new(OFFICIAL_SESSIONS_FOLDER).is_dir() {
        ...
        std::fs::rename(OFFICIAL_SESSIONS_FOLDER, TEMP_SESS_BACKUP_FOLDER).unwrap();
    }

    println!("Recovering last saved session ..");
    for (absolute_from, absolute_to) in
        pzlib::rdr::read_dir_recursive(util::get_session_path(args.n, PZLOAD_SESSIONS_FOLDER))
            .unwrap()
            .map(|r| r.unwrap())
            .map(|e| e.path())
            .map(|absolute_from| {
            ...
            .map(|(absolute_from, relative_dest)| {
                let absolute_to =
                    std::path::Path::new(OFFICIAL_SESSIONS_FOLDER).join(relative_dest);
